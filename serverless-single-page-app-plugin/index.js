	...
	domainInfo() {
		...
		return provider
			...
			.then((result) => {
				...
				const output = outputs.find(entry => entry.OutputKey === 'LucasKlaassenCloudFrontDistributionOutput');
				if (output.OutputValue) {
					this.serverless.cli.log(`LucasKlaassen Domain: ${output.OutputValue}`);
				} else {
					this.serverless.cli.log('LucasKlaassen Domain: Not Found');
				}
			});
	}
}
